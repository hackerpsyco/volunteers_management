...
async function run() {
  const taskName = "Research On - Datacenters, Cloud Storage, and Azure Storage Services";
  const { data, error } = await supabase
    .from('student_task_feedback')
    .select('id, student_id, task_name, deadline, created_at, academic_year')
  ...
  data?.forEach((row, i) => {
    ...
    console.log(`  Academic Year: ${row.academic_year}`);
  });
}
